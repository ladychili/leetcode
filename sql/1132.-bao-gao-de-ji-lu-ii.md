...
1. actions left join removals 可得知每个帖子是否有删除记录
2. 按日期聚合，计算出当日被举报的帖子数 n\_report 和有删除记录的帖子数 n\_remove
3. 取平均，乘以100，保留两位小数

```sql
select round(AVG(n_remove/n_report)*100,2) average_daily_percent
from 
    (select a.action_date, 
        count(distinct a.post_id) n_report, 
        count(distinct b.post_id) n_remove
    from Actions a

...
现在有一个工厂网站由几台机器运行，每台机器上运行着相同数量的进程. 请写出一条SQL计算**每台机器各自完成一个进程任务的平均耗时**.
...
FROM
(select *, timestamp-lag(timestamp,1,0) over(order by machine_id , process_id , activity_type) runtime
from Activity) t

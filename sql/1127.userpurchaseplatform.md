...
统计每天每个平台的交易额和用户数, "双平台同时消费的用户"分开统计. 难点: 1. 需要为_platform_字段构造一个'both'值, 这里用了`CASE WHEN END`语句 2. 即使某天没有"双平台同时消费的用户"也要显示

* 这里在"table aa"里组合了所有 _spend\_date_ 和 _platform_, 然后`LEFT JOIN`保证每天都有3个platform项
